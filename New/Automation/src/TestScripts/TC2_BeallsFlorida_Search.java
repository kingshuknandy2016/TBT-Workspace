...
public class TC2_BeallsFlorida_Search {
	...
	public TC2_BeallsFlorida_Search ()throws Exception{
		sel=Main.sellaunch;
//		xp = Main.xpath;	
	}
     
     public void TC2_BeallsFlorida_Search()throws Exception{
    	 try
 		{	
    	 System.out.println("*******----- TC2_BeallsFlorida_Search -----*******");	
    	Genericscripts gs = new Genericscripts(getClass().getSimpleName());
    	sel.windowMaximize();
		...
		if(sel.isElementPresent("//*[@id='SimpleSearchForm_SearchTerm']"))
		{
			for (int i = 1; i < rc; i++) {
				String searchData = s.getCell(0, i).getContents();
				System.out.println(searchData);
				
				gs.typeIntoEditBox("//*[@id='SimpleSearchForm_SearchTerm']", searchData);
		...
		{
			gs.writeFailLog("Search Element Not Found");
		}
